...
### Data flow
1. `stores/dataset.js:loadDatasetManifest()` fetches + validates `/data/datasets.json` into `availableDatasetsStore`; `currentDatasetStore` holds the active dataset and persists the selection to IndexedDB.
2. `+page.svelte` awaits `loadDatasetManifest()` then `restoreDataset()` on mount, then reacts to `currentDatasetStore` changes: it clears taxa/filters/folder, calls `csv.js:loadSpeciesData(csvPath)`, and restores that dataset's saved folder handle.
3. `taxa.js:taxaStore` holds the parsed data; `filteredSpecies` is a derived store driven by `filterStore`.
4. `folder.js` keys each `FileSystemDirectoryHandle` by dataset (`imageFolderHandle:{id}`) so folders are remembered per-dataset. A one-time migration moves any legacy single-key handle into the Ankarafantsika slot (the only dataset that existed under the old single-key layout).
5. Images are loaded on-demand via the File System Access API (Chrome only) — the app never bundles images.

### Key directories
```
src/lib/
  datasets.js        — dataset manifest schema: validation (parseManifest) + per-kind defaults
...
static/data/         — datasets.json manifest + one CSV per dataset (copied verbatim to build/data/ at build time)
scripts/             — Node-only tooling outside the SvelteKit build (manifest loader, edition packaging, thumbnail + basemap generation, data/geo verification, Phase B v2 KB build)
```
...
npm run package -- --datasets=<id>[,<id>]  # stage dist/<edition>/ + zip from the current build/
```
...
The registry is the **runtime manifest** `static/data/datasets.json` (→ `build/data/datasets.json`), not compiled-in source. It is `{ manifestVersion, defaultDatasetId, datasets: [...] }`; each entry is `{ id, label, csvPath, kind, allowBarcodeLess?, defaults? }`. `defaultDatasetId` picks the one loaded on first run. `allowBarcodeLess` is true only for site checklists that deliberately retain a named taxon before any specimen image is catalogued.

`src/lib/datasets.js` keeps the schema and the filename helpers: `parseManifest` validates and normalises the manifest and is the single validator shared by the browser (`stores/dataset.js:loadDatasetManifest`) and the Node tooling (`scripts/manifest.js`). A missing or malformed manifest is a hard, visible failure — an amber sidebar banner plus a "No dataset could be loaded" grid — never a silent fallback to a built-in list, which would defeat per-edition packaging. Because the manifest is fetched at runtime like the CSVs, one build serves every edition. CSV column schema (shared across datasets):
...
3. Append a `{ id, label, csvPath, kind }` entry to `datasets` in `static/data/datasets.json`; add `allowBarcodeLess: true` only for a checklist that intentionally permits placeholder taxa, and a `defaults` block only to override its kind (below).
4. Rebuild. The sidebar selector picks it up automatically: one dataset renders as a static label, two as a segmented control, ≥3 as a dropdown.

No source edit is needed, so a deployed install can gain a dataset by dropping the CSV into `build/data/` and adding its entry to `build/data/datasets.json` — the same live-edit path the CSVs already have.

If a manifest entry points at a CSV that doesn't exist yet, switching to it logs a 404 and leaves `taxaStore` null — the grid is empty until the file is dropped in.

### Per-dataset defaults (`kind`)

Each entry declares a `kind`, which supplies the state a dataset opens on (`KIND_DEFAULTS` in
`datasets.js`); an optional `defaults` block overrides any field:

| kind | sort | mapExtent | habits |
|---|---|---|---|
| `checklist` (site flora) | `family` | `madagascar` | `null` → `DEFAULT_HABITS` (tree+shrub) |
| `monograph` (taxon revision) | `name` | `madagascar` | `[]` (habit filter off) |

Applied by the dataset-switch effect in `+page.svelte`. A monograph opens on Name sort because
Family is degenerate in a one-family dataset, and with no habit filter because its CSV carries no
`Habit` column and the woody default would silently hide material in a mixed-habit genus.
`SortPills` still falls back to `name` if a dataset makes the chosen mode degenerate, so the two
compose.

`mapExtent` accepts `madagascar` | `wio` | `global` | `auto`. Set it explicitly for a taxon with
out-of-island material: Macaranga is `wio` so its Comoros/Mayotte/Mauritius sheets (island-anchored,
and therefore hidden on the Madagascar frame) are visible on open. Prefer explicit over `auto`:
`auto` resolves the narrowest extent containing *every* point, so one bad coordinate drags the whole
frame outwards — Sarcolaenaceae's nine known-bad records (`scripts/data-quality-notes.js`) include
points in India and South America, which resolve it to the world map.

### Packaging an edition

```bash
npm run build
npm run package -- --datasets=sarcolaenaceae            # single-collection edition
npm run package -- --datasets=macaranga,sarcolaenaceae  # Kew monographs edition
```

Because the registry is a runtime manifest, one build serves every edition: `scripts/package-release.js`
copies `build/`, prunes `build/data/` to the selected CSVs, writes a manifest listing exactly those,
adds the launchers + README, and zips it (python3; skipped with a warning if absent). Output is
`dist/deepZEN-curator-<version>-<slug>/` plus `.zip`, matching the hand-assembled layout. Flags:
`--default=<id>` (opening dataset), `--out`, `--name`, `--no-zip`. It refuses unknown ids, a
`--default` outside the selection, and a missing `build/`, and verifies after staging that every
listed CSV is present and no unlisted CSV survived.
...
- **Selectable extent (Madagascar / WIO / Global)**: the basemap frames one of three extents,
  chosen by the toolbar's 3-way selector. The opening extent comes from the dataset's manifest
  `defaults.mapExtent` (see Per-dataset defaults) — `madagascar` for everything except Macaranga,
  which opens on `wio` for its island material. `detectExtent` (`geo.js`) — used only when the extent is set to `'auto'`
  ...
  (`stores/map.js`, session-only) holds the dataset's `defaults.mapExtent` (re-applied per dataset
  switch), an explicit `'wio'`/`'global'` pick, or `'auto'`; `effectiveMapExtent` + `activeMapBbox`
